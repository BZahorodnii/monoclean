...
const useGetPriceWithoutSquare = () => {
  ...
  const fetch = async (body: BodyI, track = false) => {
    ...
    const response = await fetchData(`guest/order/price`, 'POST', body, 'en', 'https://app.monoclean.ca/api/');
    // const response = {
    //   code: 200,
    //   order: {
    //     bonus_fund_fee: 0.81, 
...
export default useGetPriceWithoutSquare;
