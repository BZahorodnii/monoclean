...
import { becomeACleanerSchema } from '../../../schemas'
...
const BecomeACleaner: React.FC = () => {
  ...
  useEffect(() => {
    if (appData?.dialogOpen === 'becomeACleaner') {
  ...
  const formik = useFormik({
    ...
    validationSchema: becomeACleanerSchema,
    onSubmit: values => {
      const { email, full_name, phone_number, experience_level } = values;
      ...
      sendForm('becomeCleaner', formData);
    },
  ...
  return (
    <div>
      <Modal
        isOpen={appData?.dialogOpen === 'becomeACleaner'}
      ...
      >
        ...
        {Boolean(sendFormData) ? (
          ...
          <>
            <h2 className={classnames('h2', styles.title)}>{convertTranslation(appData?.strings)?.become_a_cleaner}</h2>
            <form noValidate onSubmit={formik.handleSubmit}>
              ...
              <div className={classnames(styles.inputWrapper, styles.inputsFlex)}>
                <TextInput
                ...
                <TextInput
                  ...
                  onChange={handleChange}
                  error={formik.errors.phone_number || null}
                  type="tel"
                  name="phone_number"
                  placeholder={convertTranslation(appData?.strings)?.phone_number}
                />
              </div>
              <div className={styles.inputWrapper}>
                <Multiselect
                  ...
                  placeholder={convertTranslation(appData?.strings)?.area_of_work}
                />
              </div>
              <div className={styles.lastInputWrapper}>
                <TextInput
                  ...
                  onChange={handleChange}
                  error={formik.errors.experience_level || null}
                  type="text"
                  name="experience_level"
                  placeholder={convertTranslation(appData?.strings)?.experience_level}
                />
              </div>
              <div className={styles.buttonWrapper}>
                <Button
                  ...
                  type="submit"
                  text={convertTranslation(appData?.strings)?.become_a_cleaner}
                  fullWidthMobile
